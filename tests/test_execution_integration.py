...
class TestExecutionIntegration(unittest.TestCase):
    ...
    def test_execute_load_distribution(self) -> None:
        """Test load distribution mode."""
        files = {}
        for i in range(1, 7):
            files[f"test_file{i}.py"] = textwrap.dedent(f"""
                def test_f{i}_t1():
                    assert True
        ...
        with create_test_project(files) as project_path:
            returncode, stdout, stderr = run_rtest(["-n", "3", "--dist", "load"], cwd=str(project_path))

            self.assertEqual(returncode, 0, "All tests should pass")
            output = stdout + stderr
            self.assertTrue("12 passed" in output or "Worker" in output)

    def test_execute_loadscope_distribution(self) -> None:
        """Test loadscope distribution mode."""
        files = {
            "test_classes.py": textwrap.dedent("""
                class TestClassA:
                    def test_a1(self):
                        assert True
        ...
        with create_test_project(files) as project_path:
            returncode, stdout, stderr = run_rtest(["-n", "3", "--dist", "loadscope"], cwd=str(project_path))

            self.assertEqual(returncode, 0, "All tests should pass")
            output = stdout + stderr
            self.assertTrue("5 passed" in output or "Worker" in output)

    def test_execute_loadfile_distribution(self) -> None:
    ...
    def test_execute_worksteal_distribution(self) -> None:
        """Test worksteal distribution mode."""
        files = {
            "test_worksteal.py": textwrap.dedent("""
                import time
        ...
        with create_test_project(files) as project_path:
            returncode, stdout, stderr = run_rtest(["-n", "2", "--dist", "worksteal"], cwd=str(project_path))

            self.assertEqual(returncode, 0, "All tests should pass")
            output = stdout + stderr
            self.assertTrue("4 passed" in output or "Worker" in output)

    def test_execute_no_distribution(self) -> None:
        ...
        with create_test_project(files) as project_path:
            returncode, stdout, stderr = run_rtest(["-n", "3", "--dist", "no"], cwd=str(project_path))

            self.assertEqual(returncode, 0, "All tests should pass")
            output = stdout + stderr
            # All tests should run in a single batch
            self.assertIn("6 passed", output)
    ...
    def test_execute_maxprocesses_limit(self) -> None:
        """Test maxprocesses limit."""
        files = {}
        for i in range(1, 21):
            files[f"test_proc_{i}.py"] = textwrap.dedent(f"""
                def test_p{i}_t1():
                    assert True
            """)

        with create_test_project(files) as project_path:
            returncode, stdout, stderr = run_rtest(["-n", "10", "--maxprocesses", "5"], cwd=str(project_path))
