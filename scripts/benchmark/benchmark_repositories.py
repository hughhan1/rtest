...
class RepositoryManager:
    ...
    def setup_repository(self, repo_config: RepositoryConfig, repo_path: Path) -> bool:
        ...
        logger.info(f"Running: {pip_executable} install -r requirements.txt")
        result = run_command([str(pip_executable), "install", "-r", "requirements.txt"], str(repo_path))
        logger.info(f"pip install exit code: {result.returncode}")
        if result.stdout:
        ...
        # Log installed packages to help debug dependency issues
        logger.info("Checking installed packages...")
        list_result = run_command([str(pip_executable), "list"], str(repo_path))
